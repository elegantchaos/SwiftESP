...
public struct RecordMetadata: Codable {
  ...
  init(
    header: RecordHeader, fields: UnpackedFields? = nil, fieldOrder: [Tag]? = nil,
    originalData: Bytes? = nil, children: [RecordProtocol]? = nil
  ) {
    self.header = header
    self.fields = fields
...
extension RecordProtocol {
  ...
  public var namedID: String? {
    guard let identified = self as? IdentifiedRecord, let id = identified.editorID else {
      return nil
    }
    return id
  }
...
extension RecordProtocol {
  public var description: String { return "«\(fullName)»" }
}
...
public protocol IdentifiedRecord: RecordProtocol {
