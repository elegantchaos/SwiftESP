...
extension Processor {
  fileprivate func recordData<I: AsyncByteSequence>(bytes: I) -> AsyncThrowingIteratorMapSequence<
    I, RecordData
  > {
    let records = bytes.iteratorMap { iterator -> RecordData in
      ...
      if !isGroup, header.isCompressed {
        ...
        let decompressed = try ZlibArchive.unarchive(
          archive: Data(bytes: bytes, count: bytes.count))
        data = LoadedRecordData(data: decompressed.littleEndianBytes)
        if data.data.count != decompressedSize {
          compressionChannel.log(
            "Size mismatch: only got \(data.data.count) bytes, expected \(decompressedSize).")
        }
      } else {
  ...
  fileprivate func records<I: AsyncByteSequence>(bytes: I, processChildren: Bool)
    -> AsyncThrowingMapSequence<AsyncThrowingIteratorMapSequence<I, RecordData>, RecordProtocol>
  {
  ...
  fileprivate func record(from record: RecordData, processChildren: Bool) async throws
    -> RecordProtocol
  {
    if record.isGroup {
      ...
      let fields = try await decodedFields(
        type: record.type, header: record.header, data: record.data)
  ...
  fileprivate func fields<I>(
    bytes: inout I, types: FieldTypeMap, inRecord recordType: Tag,
    withHeader recordHeader: RecordHeader
  ) -> AsyncThrowingIteratorMapSequence<I, Field> where I: AsyncSequence, I.Element == Byte {
    let sequence = bytes.iteratorMap { iterator -> Field in
      let header = try await Field.Header(&iterator)
      do {
        let data = try await iterator.next(Bytes.self, count: Int(header.size))
        return try await self.inflate(
          header: header, data: data, types: types, inRecord: recordType, withHeader: recordHeader)
      } catch {
        throw error
      }
    }
  ...
  fileprivate func decodedFields(type: Tag, header: RecordHeader, data: RecordDataProvider)
    async throws -> DecodedFields
  {
  ...
  fileprivate func inflate(
    header: Field.Header, data: Bytes, types: FieldTypeMap, inRecord recordType: Tag,
    withHeader recordHeader: RecordHeader
  ) async throws -> Field {
    if let type = types.fieldType(forTag: header.type) {
      do {
        let decoder = FieldDecoder(
          header: header, data: data, inRecord: recordType, withHeader: recordHeader)
  ...
  fileprivate func save(records: [RecordProtocol], to url: URL) async throws {
    var index = 0
  ...
  fileprivate func save(record: RecordProtocol, index: Int, asJSONTo url: URL) async throws {
  ...
  fileprivate func save(group: GroupRecord, index: Int, asJSONTo url: URL) async throws {
    let name = String(format: "%04d %@", index, group.name)
    let groupURL = url.appendingPathComponent(name).appendingPathExtension(
      GroupRecord.fileExtension)
  ...
  fileprivate func pack(records: [RecordProtocol]) throws -> Data {
  ...
  fileprivate func loadRecords(from url: URL) throws -> [RecordProtocol] {
